...
class Selectors
{
	...
	public static function append_selector($selector, $to_append)
	{
		...
		return new Script\Literals\String(join(', ', $appended));
	}
	...
	{
		if (!$from || ($from instanceof Script\Literals\String && $from->value==='all')) {
			$from=new Script\Literals\Number(1);
			$to=new Script\Literals\Number(6);
			}
		elseif ($from && !$to) {
			$to=$from;
			$from=new Script\Literals\Number(1);
	...
	public static function enumerate($prefix, $from, $to, $separator=NULL)
	{
		...
		return new Script\Literals\String($_prefix.join(', '.$_prefix, range($from->value, $to->value)));
	}
	...
	{
		return new Script\Literals\String(join(', ', self::$defaultDisplay[$display->value]));
	}
}
