...
use Lohini\WebLoader\Filters\Sass\Script\Literals\Number,
...
class ImageSize
{
	...
	public function image_width($image_file)
	{
		...
		return new Number($image_size[0].'px');
	}
	...
	{
		$image_size=getimagesize(self::real_path($image_file));
		return new Number($image_size[1].'px');
	}
	...
	private function real_path($image_file) {
		...
		return Config::config('images_path') ?: Config::config('project_path').DIRECTORY_SEPARATOR.$path;
	}
}
