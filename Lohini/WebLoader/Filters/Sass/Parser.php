...
class Parser
{
	...
	public function __construct($options=array())
	{
		...
		if (!empty($options['extensions'])) {
			foreach ($options['extensions'] as $extension => $extOptions) {
				$configClass=__NAMESPACE__."\\Extensions\\$extension\\Config";
		...
		$defaultOptions=array(
			...
			'line' => 1,
			'line_numbers' => FALSE,
			'style' => Sass\Renderers\Renderer::STYLE_NESTED,
			'syntax' => File::SASS
			);
	 ...
	 * @param string $source name of source file or Sass source
	 * @param bool $isFile
	 * @return Sass\Tree\RootNode Root node of document tree
	 * @throws Exception
	 */
	public function parse($source, $isFile=TRUE)
	{
		if ($isFile) {
			$this->filename=File::getFile($source, $this);

			$this->syntax=\Nette\Utils\Strings::lower(pathinfo($this->filename, PATHINFO_EXTENSION));
			if ($this->syntax!==File::SASS && $this->syntax!==File::SCSS) {
				throw new Exception('Invalid syntax option');
				}

			if ($this->cache) {
				if (($cached=File::getCachedFile($this->filename))!==NULL) {
					return $cached;
					}
				}
			...
			if ($this->cache) {
				File::setCachedFile($tree, $this->filename);
				}
			return $tree;
			}
		else {
	 ...
	 * @return Sass\Tree\RootNode the root of this document tree
	 */
	private function toTree($source)
	{
		...
		unset($source);
		$root=new Sass\Tree\RootNode($this);
		$this->buildTree($root);
		return $root;
	}
	 ...
	 * @param Sass\Tree\Node $parent the node
	 * @return Sass\Tree\Node
	 */
	...
	private function getNode($node)
	{
		...
		switch (TRUE) {
			case Sass\Tree\DirectiveNode::isa($token):
				return $this->parseDirective($token, $node);
				break;
			case Sass\Tree\CommentNode::isa($token):
				return new Sass\Tree\CommentNode($token);
				break;
			case Sass\Tree\VariableNode::isa($token):
				return new Sass\Tree\VariableNode($token);
				break;
			case Sass\Tree\PropertyNode::isa(array('token'=>$token, 'syntax'=>$this->property_syntax)):
				return new Sass\Tree\PropertyNode($token, $this->property_syntax);
				break;
			case Sass\Tree\MixinDefinitionNode::isa($token):
				if ($this->syntax===File::SCSS) {
					throw new Exception('Mixin definition shortcut not allowed in SCSS', $this);
					}
				return new Sass\Tree\MixinDefinitionNode($token);
				break;
			case Sass\Tree\MixinNode::isa($token):
				if ($this->syntax===File::SCSS) {
					throw new Exception('Mixin include shortcut not allowed in SCSS', $this);
					}
				return new Sass\Tree\MixinNode($token);
				break;
			default:
				return new Sass\Tree\RuleNode($token);
				break;
			} // switch
	}
	...
	{
		return $this->syntax===File::SASS? $this->sass2Token() : $this->scss2Token();
	}
	...
	private function sass2Token()
	{
		...
		while (is_null($token) && !empty($this->source)) {
			...
			elseif (substr($statement, -1)===Sass\Tree\RuleNode::CONTINUED) {
	 ...
	 * @param Sass\Tree\Node $parent parent node
	 * @return Sass\Tree\Node a Sass directive node
	 * @throws Exception
	 */
	private function parseDirective($token, $parent)
	{
		switch (Sass\Tree\DirectiveNode::extractDirective($token)) {
			case '@extend':
				return new Sass\Tree\ExtendNode($token);
				break;
			case '@mixin':
				return new Sass\Tree\MixinDefinitionNode($token);
				break;
			...
			case '@import':
				...
				return new Sass\Tree\ImportNode($token);
				break;
			case '@each':
				return new Sass\Tree\EachNode($token);
				break;
			case '@for':
				return new Sass\Tree\ForNode($token);
				break;
			case '@if':
				return new Sass\Tree\IfNode($token);
				break;
			case '@else': // handles else and else if directives
				return new Sass\Tree\ElseNode($token);
				break;
			case '@do':
			case '@while':
				return new Sass\Tree\WhileNode($token);
				break;
			case '@debug':
				return new Sass\Tree\DebugNode($token);
				break;
			case '@warn':
	 ...
	 * @throws Exception if the indent is mixed or the indent character can not be determined
	 */
