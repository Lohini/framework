...
extends Literal
{
	...
	public function __construct($colour)
	{
		if (is_string($colour)) {
			...
			if ($colour===self::TRANSPARENT) {
				...
				if (empty($matches)) {
					throw new ColourException('Invalid Colour string', Parser::$context->node);
					}
	...
	public function op_plus($other)
	{
		if ($other instanceof Number) {
			if (!$other->isUnitless()) {
				throw new ColourException('Number must be a unitless number', Parser::$context->node);
				}
		...
		elseif (!$other instanceof Colour) {
			throw new ColourException('Argument must be a Colour or Number', Parser::$context->node);
			}
		else {
	...
	private function assertValid($colour, $all=TRUE)
	{
		if (array_key_exists('red', $colour) || array_key_exists('green', $colour) || array_key_exists('blue', $colour)) {
			if (array_key_exists('hue', $colour) || array_key_exists('saturation', $colour) || array_key_exists('lightness', $colour)) {
				throw new ColourException('Colour can not have HSL and RGB keys specified', Parser::$context->node);
				}
			if ($all && (!array_key_exists('red', $colour) || !array_key_exists('green', $colour) || !array_key_exists('blue', $colour))) {
				throw new ColourException('Colour must have all RGB keys specified', Parser::$context->node);
				}
			return 'rgb';
			}
		elseif (array_key_exists('hue', $colour) || array_key_exists('saturation', $colour) || array_key_exists('lightness', $colour)) {
			if ($all && (!array_key_exists('hue', $colour) || !array_key_exists('saturation', $colour) || !array_key_exists('lightness', $colour))) {
				throw new ColourException('Colour must have all HSL keys specified', Parser::$context->node);
				}
			return 'hsl';		
			}
		elseif ($all && sizeof($colour)<3) {
			throw new ColourException('Colour array must have at least 3 elements', Parser::$context->node);
			}
	}
