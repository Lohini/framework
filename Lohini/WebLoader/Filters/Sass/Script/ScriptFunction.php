...
class ScriptFunction
{
	 ...
	 * @return mixed the value of this Function
	 * @throws FunctionException if function is undefined
	 */
	public function perform()
	{
		...
		foreach (Parser::$context->node->parser->function_paths as $path) { // TODO: remake to use RobotLoader
			...
			foreach (array_slice(scandir($path), 2) as $file) {
				...
				if (is_file($filename)) {
					...
					$class=__NAMESPACE__."\Extensions\\{$_class}Functions".ucfirst(substr($file, 0, -4));
		...
		if (method_exists(__NAMESPACE__.'\Functions', $name)) {
