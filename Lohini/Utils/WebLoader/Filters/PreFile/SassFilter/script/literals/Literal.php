...
abstract class Literal
{
	...
	public function __get($name)
	{
		...
		if (method_exists($this, $getter)) {
			...
			throw new Sass\LiteralException("No getter function for $name", Sass\ScriptParser::$context->node);
			}
	}
	...
	{
		throw new Sass\LiteralException('Child classes must override this method', Sass\ScriptParser::$context->node);
	}
	...
	{
		return new Sass\String($this->toString().$other->toString());
	}
	...
	{
		throw new Sass\LiteralException(get_class($this).' does not support Modulus.', Sass\ScriptParser::$context->node);
	}
	...
	{
		throw new Sass\LiteralException(get_class($this).' does not support Bitwise AND.', Sass\ScriptParser::$context->node);
	}
	...
	{
		throw new Sass\LiteralException(get_class($this).' does not support Bitwise OR.', Sass\ScriptParser::$context->node);
	}
	...
	{
		throw new Sass\LiteralException(get_class($this).' does not support Bitwise XOR.', Sass\ScriptParser::$context->node);
	}
	...
	{
		throw new Sass\LiteralException(get_class($this).' does not support Bitwise NOT.', Sass\ScriptParser::$context->node);
	}
	...
	{
		throw new Sass\LiteralException(get_class($this).' does not support Bitwise Shift Left.', Sass\ScriptParser::$context->node);
	}
	...
	{
		throw new Sass\LiteralException(get_class($this).' does not support Bitwise Shift Right.', Sass\ScriptParser::$context->node);
	}
	 ...
	 * @return Boolean Sass\Boolean object with the value true if this or
	 * other, but not both, are true, false if not
	 */
	public function op_xor($other)
	{
		return new Sass\Boolean($this->toBoolean() xor $other->toBoolean());
	}
	...
	{
		return new Sass\Boolean(!$this->toBoolean());
	}
	...
	{
		return new Sass\Boolean($this->value>$other->value);
	}
	...
	{
		return new Sass\Boolean($this==$other);
	}
	...
	{
		return new Sass\Boolean(!$this->op_eq($other)->toBoolean());
	}
	...
	public static function assertType($literal, $type)
	{
		if (!$literal instanceof $type) {
			throw new Sass\ScriptFunctionException(($literal instanceof Sass\Literal? $literal->typeOf : 'literal')." must be a $type", Sass\ScriptParser::$context->node);
			}
	}
	 ...
	 */
	 public static function assertInRange($literal, $min, $max, $units='')
	 {
	 	 if ($literal->value<$min || $literal->value>$max) {
			throw new Sass\ScriptFunctionException("$literal->typeOf must be between $min$units and $max$units inclusive", Sass\ScriptParser::$context->node);
			}
	}
