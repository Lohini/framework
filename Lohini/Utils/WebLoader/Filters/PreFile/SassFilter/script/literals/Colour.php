 ...
 * representation Ă˘â‚¬â€ś for example, Sass\Colour::red for an HSL color Ă˘â‚¬â€ś that
 ...
 * channel is modified using Sass\Colour::with(), the cached RGB and HSL values
 * are retained.
 * 
...
extends Literal
{
	...
	*/
	...
	public function __construct($colour)
	{
		if (is_string($colour)) {
			...
			if ($colour===self::TRANSPARENT) {
				...
				if (empty($matches)) {
					throw new Sass\ColourException('Invalid Sass\Colour string', Sass\ScriptParser::$context->node);
					}
		...
		elseif (is_array($colour)) {
			...
			throw new Sass\ColourException('Colour must be a array', Sass\ScriptParser::$context->node);
			}
	}
	...
	{
		if ($other instanceof Sass\Number) {
			if (!$other->isUnitless()) {
	...
	*/
	private function assertValid($colour, $all=TRUE)
	{
		if (array_key_exists('red', $colour) || array_key_exists('green', $colour) || array_key_exists('blue', $colour)) {
			if (array_key_exists('hue', $colour) || array_key_exists('saturation', $colour) || array_key_exists('lightness', $colour)) {
				throw new Sass\ColourException('Sass\Colour can not have HSL and RGB keys specified', Sass\ScriptParser::$context->node);
				}
			if ($all && (!array_key_exists('red', $colour) || !array_key_exists('green', $colour) || !array_key_exists('blue', $colour))) {
				throw new Sass\ColourException('Sass\Colour must have all RGB keys specified', Sass\ScriptParser::$context->node);
				}
			return 'rgb';
			}
		elseif (array_key_exists('hue', $colour) || array_key_exists('saturation', $colour) || array_key_exists('lightness', $colour)) {
			if ($all && (!array_key_exists('hue', $colour) || !array_key_exists('saturation', $colour) || !array_key_exists('lightness', $colour))) {
				throw new Sass\ColourException('Sass\Colour must have all HSL keys specified', Sass\ScriptParser::$context->node);
				}
			return 'hsl';		
			}
		elseif ($all && sizeof($colour)<3) {
			throw new Sass\ColourException('Sass\Colour array must have at least 3 elements', Sass\ScriptParser::$context->node);
			}
	}
	 ...
	 * @param string $subjectthe subject string
