...
/**
 * ScriptParser class.
 * Parses Sass\Script. Sass\Script is lexed into {@link http://en.wikipedia.org/wiki/Reverse_Polish_notation Reverse Polish notation} by the Sass\ScriptLexer and
...
class ScriptParser
{
	...
	/** @var ScriptLexer the lexer object */
	private $lexer;


	public function __construct()
	{
		$this->lexer=new Sass\ScriptLexer($this);
	}
	...
	public function evaluate($expression, $context, $environment=self::DEFAULT_ENV)
	{
		...
		while (count($tokens)) {
			...
			if ($token instanceof Sass\ScriptFunction) {
				array_push($operands, $token->perform());
				}
			elseif ($token instanceof Sass\Literal) {
				if ($token instanceof Sass\String) {
					$token=new Sass\String($this->interpolate($token->toString(), self::$context));
					}
	...
	public function parse($expression, $context, $environment=self::DEFAULT_ENV)
	{
		...
		foreach ($tokens as $i => $token) {
			...
			if (empty($token)) {
				if ($i>0
					&& (!$tokens[$i-1] instanceof Sass\ScriptOperation || $tokens[$i-1]->operator===Sass\ScriptOperation::$operators[')'][0])
					&& (!$tokens[$i+1] instanceof Sass\ScriptOperation || $tokens[$i+1]->operator===Sass\ScriptOperation::$operators['('][0])
					) {
					$token=new Sass\ScriptOperation(Sass\ScriptOperation::$defaultOperator, $context);
					}
				else {
					continue;
					}
				}
			elseif ($token instanceof Sass\ScriptVariable) {
				$token=$token->evaluate($context);
				$environment=self::DEFAULT_ENV;
				}

			// If the token is a number or function add it to the output queue.
 			if ($token instanceof Sass\Literal || $token instanceof Sass\ScriptFunction) {
 				if ($environment===self::CSS_PROPERTY && $token instanceof Sass\Number && !$parenthesis) {
					$token->inExpression=FALSE; 
 					}
				array_push($outputQueue, $token);
				}
			// If the token is an operation
			elseif ($token instanceof Sass\ScriptOperation) {
				// If the token is a left parenthesis push it onto the stack.
				if ($token->operator==Sass\ScriptOperation::$operators['('][0]) {
					array_push($operatorStack, $token);
					$parenthesis++;
					}
				// If the token is a right parenthesis:
				elseif ($token->operator==Sass\ScriptOperation::$operators[')'][0]) {
					$parenthesis--;
					while ($c=count($operatorStack)) {
						// If the token at the top of the stack is a left parenthesis
						if ($operatorStack[$c-1]->operator==Sass\ScriptOperation::$operators['('][0]) {
				...
				// the token is an operator, o1, so:
					...
					while ($c=count($operatorStack)) {
						...
						if (($operation->operator==Sass\ScriptOperation::$operators['('][0])
