...
implements \ArrayAccess
{
	...
	/** @var \BailIff\Components\DataGrid\Columns\ActionColumn */
	 ...
	 * @return \ArrayIterator
	 */
	public function getColumns($type='BailIff\Components\DataGrid\Columns\IColumn')
	{
	 ...
	 * @return ArrayIterator
	 */
	public function getFilters($type='BailIff\Components\DataGrid\Filters\IColumnFilter')
	{
	...
	public function getActions($type='BailIff\Components\DataGrid\IAction')
	{
		...
		foreach ($this->getColumns('BailIff\Components\DataGrid\Columns\ActionColumn') as $column) {
			if ($column->hasAction()) {
	...
	{
		if ($value<0) {
			throw new \InvalidArgumentException("Parameter must be non-negative number, '$value' given.");
			}
	 ...
	 * @param \BailIff\Components\DataGrid\Renderers\IRenderer
	 */
	public function setRenderer(\BailIff\Components\DataGrid\Renderers\IRenderer $renderer)
	{
	...
	public function getRenderer()
	{
		if ($this->renderer===NULL) {
			$this->renderer=new \BailIff\Components\DataGrid\Renderers\Conventional;
			}
	...
	public function render()
	{
		if (!$this->wasRendered) {
			...
			if (!$this->hasColumns() || (count($this->getColumns('BailIff\Components\DataGrid\Columns\ActionColumn'))==count($this->getColumns()))) {
	...
	protected function createComponentForm($name)
	{
		...
		$form=new \BailIff\Application\UI\Form($this, $name);
	...
	protected function regenerateFormControls()
	{
		...
		if ($this->hasFilters()) {
			...
			foreach ($this->getFilters() as $filter) {
				if ($filter instanceof \BailIff\Components\DataGrid\Filters\SelectboxFilter) {
	 ...
	 * @return \BailIff\Components\DataGrid\Columns\TextColumn
	 */
	 ...
	 * @return \BailIff\Components\DataGrid\Columns\NumericColumn
	 */
	 ...
	 * @return \BailIff\Components\DataGrid\Columns\DateColumn
	 */
	 ...
	 * @param string $name control name
	 * @param string $caption column label
	 * @return \BailIff\Components\DataGrid\Columns\CheckboxColumn
	 */
	 ...
	 * @param string $name control name
	 * @param string $caption column label
	 * @return \BailIff\Components\DataGrid\Columns\ImageColumn
	 */
	 ...
	 * @return \BailIff\Components\DataGrid\Columns\PositionColumn
	 */
	 ...
	 * @return \BailIff\Components\DataGrid\Columns\ActionColumn
	 */
	 ...
	 * @return \BailIff\Components\DataGrid\Columns\CheckboxColumn
	 */
	public function addBoolColumn($name, $caption=NULL)
	{
		return $this[$name]=new Columns\BoolColumn($caption);
	}

	/**
	 * @param \BailIff\Components\DataGrid\Columns\ActionColumn $column
	 */
	...
	{
		if (!$this->hasColumns('BailIff\Components\DataGrid\Columns\ActionColumn')) {
	...
	{
		return $this->getSession()->getSection('BailIff.DataGrid/'.$this->getName().'/states');
	}
