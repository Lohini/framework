...
extends Literal
{
	 ...
	 * in: inches â€” 1 inch = 2.54 centimeters
	 * cm: centimeters
	 * mm: millimeters
	 * pc: picas â€” 1 pica = 12 points
	 * pt: points â€” 1 point = 1/72nd of an inch
	 */
	...
	{
		if ($other instanceof Sass\Colour) {
			return $other->op_plus($this);
			}
		elseif (!$other instanceof Sass\Number) {
			throw new Sass\NumberException('Number must be a number', Sass\ScriptParser::$context->node);
			}
		else {
			$other=$this->convert($other);
			return new Sass\Number(($this->value+$other->value).$this->units);
			}
	}
	...
	{
		return new Sass\Number(($this->value* -1).$this->units);
	}
	...
	public function op_eq($other)
	{
		if (!$other instanceof Sass\Number) {
			return new Sass\Boolean(FALSE);
			}
		try {
			return new Sass\Boolean($this->value==$this->convert($other)->value);
			}
		catch (\Exception $e) {
			return new Sass\Boolean(FALSE);
			}		
	...
	public function op_modulo($other)
	{
		if (!$other instanceof Sass\Number || !$other->isUnitless()) {
			throw new Sass\NumberException('Number must be a unitless number', Sass\ScriptParser::$context->node);
			}
	...
	private function coercionFactor($fromUnits, $toUnits)
	{
		...
		if (sizeof($fromUnits)!==sizeof($toUnits) || !$this->areConvertable(array_merge($fromUnits, $toUnits))) {
			throw new Sass\NumberException("Incompatible units: '".join(' * ', $fromUnits)."' and '".join(' * ', $toUnits)."'", Sass\ScriptParser::$context->node);
			}
	...
	{
		if  (!$this->isInt()) {
			throw new Sass\NumberException("Not an integer: $this->value", Sass\ScriptParser::$context->node);
			}
