...
extends Node
{
	...
	public function __construct($token)
	{
		...
		$this->addSelectors($matches[Sass\RuleNode::SELECTOR]);
	}
	...
	public function render()
	{
		...
		foreach ($this->children as $child) {
			$child->parent=$this;
			if ($child instanceof Sass\RuleNode) {
				$rules.=$child->render();
				}
			else {
	...
	protected function getParentSelectors($context)
	{
		...
		while (!$ancestor instanceof Sass\RuleNode && $ancestor->hasParent()) {
			$ancestor=$ancestor->parent;
			}
		if ($ancestor instanceof Sass\RuleNode) {
	...
	private function resolveParentReferences($selector, $context)
	{
		...
		if (!count($this->parentSelectors)) {
			throw new Sass\RuleNodeException('Can not use parent selector ('.self::PARENT_REFERENCE.') when no parent selectors', $this);
			}
