...
extends Node
{
	...
	public function __construct($token)
	{
		...
		if (!preg_match(self::MATCH, $token->source, $matches)) {
			throw new Sass\ForNodeException('Invalid @for directive', $this);
			}
		$this->variable=$matches[self::VARIABLE];
		$this->from=$matches[self::FROM];
		$this->to=$matches[self::TO];
		$this->inclusive=($matches[self::INCLUSIVE]===Sass\ForNode::IS_INCLUSIVE);
	...
	public function parse($context)
	{
		...
		for ($i=$from; ($from<$to? $i<$to : $i>$to); $i=$i+$step) {
			$context->setVariable($this->variable, new Sass\Number($i));
