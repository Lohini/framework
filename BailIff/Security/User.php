...
implements \Nette\Http\IUser
{
	...
	{
		$session=$this->getSessionSection(FALSE);
		if ($session && $session->identity && $session->identity instanceof \BailIff\Database\Models\Entities\Identity) {
			$dc=$this->context->sqldb;
			return $session->identity=$session->identity->load($dc);
			}
		return NULL;
	}
	...
	protected function getSessionSection($need)
	{
		...
		if (!$session->authenticated) {
			unset(
				$session->expireTime,
				$session->expireDelta,
				$session->expireIdentity,
				$session->expireBrowser,
				$session->browserCheck,
				$session->authTime
				);
			}
