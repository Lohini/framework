...
{
	/** @var \Lohini\Database\Doctrine\Forms\EntityMapper */
	private $mapper;
	...
	public function setUp()
	{
		...
		$this->mapper=new \Lohini\Database\Doctrine\Forms\EntityMapper($this->getDoctrine());
	}
	...
	public function testItemsControlLoading_FieldNamePairs(IControl $itemsControl)
	{
		...
		$relatedDao=$this->getMockBuilder('Lohini\Database\Doctrine\Dao')
	...
	public function testItemsControlSaving_FieldNamePairs_FromRelated_Collection_MultiSelect(IControl $itemsControl, $value)
	{
		...
		$this->assertEquals(
			$value,
			array_map(function(Fixtures\RelatedEntity $related) { return $related->id; }, $children)
			);
	}
	...
	{
		return array(
			array(new CheckboxList, array(1 => FALSE, 2 => TRUE, 3 => FALSE)),
			array(new CheckboxList, array(1 => FALSE, 2 => TRUE, 3 => TRUE)),
			);
	}
	 ...
	 * @param array $value
	 */
	public function _testItemsControlSaving_FieldNamePairs_FromRelated_Collection_CheckboxList(IControl $itemsControl, $value)
	{
		// tested control dependencies
		$entity=new Fixtures\RootEntity('Nikol, ty dlouho sama nebudeš!');
		$related[]=new Fixtures\RelatedEntity($a='Moravcová rozepnula živůtek');
	...
	public function testLoading_OnLoadEvent_Entity()
	{
		...
		$calls=array();
		$container->onLoad[]=function() use (&$calls) { $calls[]=func_get_args(); };
	...
	/**
	 * @param \Lohini\Database\Doctrine\Forms\IObjectContainer $container
	 * @param bool $submitted
	 * @return Form
	 */
	private function attachContainer(\Lohini\Database\Doctrine\Forms\IObjectContainer $container, $submitted=FALSE)
	{
	...
	public function testLoading_RelatedCollection()
	{
		...
		$form->addMany(
			'children',
			function(EntityContainer $container) { $container->addText('name'); }
			);
	...
	public function testSaving_OnSaveEvent_Entity()
	{
		...
		$calls=array();
		$container->onSave[]=function() use (&$calls) { $calls[]=func_get_args(); };
	...
	private function prepareChildrenContainer($entity)
	{
		...
		$form->addMany(
			...
			->setEntityFactory(
				function($daddy) { return new \Lohini\Tests\Database\Doctrine\Forms\Fixtures\RelatedEntity(NULL, $daddy); }
				);
	 ...
	 *
	 * @param Fixtures\RootEntity $entity
	 */
	public function _testSaving_RelatedCollection_UpdatingExisting(Fixtures\RootEntity $entity)
	{
	 ...
	 * @param Fixtures\RootEntity $entity
	 */
	public function _testSaving_RelatedCollection_Appending(Fixtures\RootEntity $entity)
	{
		list($form, $id)=$this->prepareChildrenContainer($entity);
	 ...
	 * @param Fixtures\RootEntity $entity
	 */
	public function _testSaving_RelatedCollection_RemovingAndAddingNew(Fixtures\RootEntity $entity)
	{
		list($form, $id)=$this->prepareChildrenContainer($entity);
		...
		$this->assertInstanceOf('Lohini\Database\Doctrine\Dao', $call[2]);
	 ...
	 * @param Fixtures\RootEntity $entity
	 */
	public function _testSaving_RelatedCollection_RemovingWithoutSignal(Fixtures\RootEntity $entity)
	{
		list($form, $id)=$this->prepareChildrenContainer($entity);
	...
	public function _testSaving_RelatedCollection_CreatingNew()
	{
		$entity=new Fixtures\RootEntity('Sofia Vergara');
		list($form, $id)=$this->prepareChildrenContainer($entity);
