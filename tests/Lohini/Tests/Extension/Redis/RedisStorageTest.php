...
use Lohini\Extension\Redis\RedisStorage,
	Lohini\Extension\Redis\RedisJournal,
	Nette\Caching\Cache;
...
extends \Lohini\Testing\TestCase
{
	...
	private $client;
	/** @var \Lohini\Extension\Redis\RedisStorage */
	private $storage;
	...
	public function testCallbacks()
	{
		...
		$cache->save(
			$key,
			$value,
			array(Cache::CALLBACKS => array(array($cb, 1)))
			);
		...
		$cache->save(
			$key,
			$value,
			array(Cache::CALLBACKS => array(array($cb, 0)))
			);
	...
	public function testDependentItems()
	{
		...
		$cache->save(
			$key,
			$value,
			array(Cache::ITEMS => array('dependent'))
			);
		$this->assertTrue(isset($cache[$key]), 'Is cached?');
	...
	public function testLoadOrSave()
	{
		...
		$res=$cache->load(
				$key,
				function(& $dp) use ($value) {
					$dp=array(Cache::EXPIRATION => time()+2);
					return $value;
				}
				);
	...
	public function testPriority()
	{
		...
		$cache->save(
			'key1',
			'value1',
			array(Cache::PRIORITY => 100)
			);
		$cache->save(
			'key2',
			'value2',
			array(Cache::PRIORITY => 200)
			);
		$cache->save(
			'key3',
			'value3',
			array(Cache::PRIORITY => 300)
			);
		$cache['key4']='value4';
	...
	public function testTags()
	{
		...
		$cache->save(
			'key1',
			'value1',
			array(Cache::TAGS => array('one', 'two'))
			);
		$cache->save(
			'key2',
			'value2',
			array(Cache::TAGS => array('one', 'three'))
			);
		$cache->save(
			'key3',
			'value3',
			array(Cache::TAGS => array('two', 'three'))
			);
		$cache['key4']='value4';
