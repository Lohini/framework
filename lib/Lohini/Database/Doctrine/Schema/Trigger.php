...
/**
 * @method \Lohini\Database\Doctrine\Schema\Trigger insert(string $table, array $values)
 * @method \Lohini\Database\Doctrine\Schema\Trigger insertSelect(string $targetTable, \Doctrine\DBAL\Schema\Table $sourceTable)
 * @method \Lohini\Database\Doctrine\Schema\Trigger update(string $table, array $pairs, string $where=NULL)
 * @method \Lohini\Database\Doctrine\Schema\Trigger declare(string $variable, string $type)
 * @method \Lohini\Database\Doctrine\Schema\Trigger set(string $variable, string $value)
 * @method \Lohini\Database\Doctrine\Schema\Trigger if(string $condition, callback $statement)
 *
 * @method static \Lohini\Database\Doctrine\Schema\Trigger beforeInsert(string $table, string $name)
 * @method static \Lohini\Database\Doctrine\Schema\Trigger afterInsert(string $table, string $name)
 * @method static \Lohini\Database\Doctrine\Schema\Trigger beforeUpdate(string $table, string $name)
 * @method static \Lohini\Database\Doctrine\Schema\Trigger afterUpdate(string $table, string $name)
 * @method static \Lohini\Database\Doctrine\Schema\Trigger beforeDelete(string $table, string $name)
 * @method static \Lohini\Database\Doctrine\Schema\Trigger afterDelete(string $table, string $name)
 */
class Trigger
	...
	public function __call($name, $args)
	{
		...
		switch ($name) {
			...
			case 'insertSelect':
				...
				$columns=array_map(function(\Doctrine\DBAL\Schema\Column $column) { return $column->getName(); }, $sourceTable->getColumns());
...
TRG;
	...
	{
		if ($words=\Nette\Utils\Strings::matchAll($name, '~((?:^|[A-Z])[a-z]+)~')) {
			$when=reset($words[0]);
