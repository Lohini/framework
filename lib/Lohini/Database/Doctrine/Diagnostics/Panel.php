 ...
 */
namespace Lohini\Database\Doctrine\Diagnostics;
/**
 * @author David Grudl
...
implements \Nette\Diagnostics\IBarPanel, \Doctrine\DBAL\Logging\SQLLogger
{
	/** @var int logged time */
	public $totalTime=0;
	/** @var array */
	...
	protected function filterTracePaths($file)
	{
		return is_file($file)
			...
			&& strpos($file, '/Lohini/Database/Doctrine/')===FALSE
			&& strpos($file, '/Lohini/Tests/')===FALSE
			&& strpos($file, '/Lohini/Testing/')===FALSE
			&& strpos($file, '/Lohini/Utils/')===FALSE
	...
	public function renderException($e)
	{
			...
			if ($invalidEntity=Strings::match($e->getMessage(), '~^Class ([^\\s]*?) is not .*? valid~i')) {
				...
				return array(
					...
					'panel' => '<p><b>File:</b> '.\Nette\Diagnostics\Helpers::editorLink($file, $errorLine).'</p>'
						.'<pre>'.BlueScreen::highlightFile($file, $errorLine).'</pre>',
					);
				}
			}
		elseif ($e instanceof \PDOException && count($this->queries)) {
			if ($this->connection!==NULL) {
				if (!$e instanceof \Lohini\Database\Doctrine\PDOException || $e->getConnection()!==$this->connection) {
					return NULL;
					}
		...
		elseif ($e instanceof \Lohini\Database\Doctrine\QueryException && $e->getQuery()!==NULL) {
			return array(
				'tab' => 'DQL',
	...
	protected function dumpQuery($query, $params, $source=NULL)
	{
		...
		$s='<p><b>Query</b></p><table><tr><td class="nette-Doctrine2Panel-sql">'
			.\Nette\Database\Helpers::dumpSql($parametrized)
			.'</td></tr></table>';
	...
	public static function formatQuery($query, array $params)
	{
		...
		if (\Nette\Utils\Validators::isList($params)) {
			...
			return implode('', \Lohini\Utils\Arrays::zipper($parts, $params));
			}
	...
	protected function highlightAnnotationLine(AnnotationException $e)
	{
		...
		if ($context['type']==='property') {
			$refl=new \Lohini\Reflection\Property($refl->getName(), $context['property']);
			$line=$refl->getLine();
			}
	 ...
	 */
	public function registerBarPanel(\Nette\Diagnostics\Bar $bar)
	{
		$bar->addPanel($this);
	}
