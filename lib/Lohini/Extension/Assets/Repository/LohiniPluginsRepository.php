...
/**
 */
final class LohiniPluginsRepository
extends PackagesRepository
{
	...
	/**
	 * Crawls registered plugins and loads javascript resources information.
	 */
	protected function loadAssets()
	{
		...
		foreach ($this->packageManager->getPackages as $package) {
			if ($package->getName()=='Plugins') {
				foreach ($package->getPlugins() as $plugin) {
					if (!file_exists($definitionFile=$plugin->getPath().'/Resources/assets.json')) {
						continue;
						}
