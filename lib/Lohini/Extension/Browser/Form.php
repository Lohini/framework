...
extends DomElement
{
	...
	{
		if ($this->method===NULL) {
			$this->method=\Lohini\Extension\Curl\Request::POST;
	...
	public function findButton($text=NULL)
	{
		...
		foreach ($this->getSubmits() as $submit) {
			...
			if ($submit->getAttribute('name')===$text) {
				return $submit;
				}
			}
	...
	public function getSubmitValues(\DOMElement $submitBy=NULL)
	{
		...
		if ($submitBy!==NULL) {
			if (!in_array($submitBy, $this->submits, TRUE)) { // todo: maybe ignore?
				throw new \Nette\InvalidArgumentException('Given button is not inside form.');
				}

			$valueRef=& static::getRef($submitValue, $this->getControlNameKeys($submitBy));
			$valueRef= $this->getSubmitControlValue($submitBy);
			}
		return \Nette\Utils\Arrays::mergeTree($submitValue, $this->getValues());
	}
	...
	public static function mergeTree($arr1, $arr2)
	{
		if ($diff=array_diff_key($arr1, $arr2)) {
			$diff=array_map(
				function($val) use ($arr1) { return array_search($val, $arr1, TRUE); },
				$diff
				);
			throw new \Nette\InvalidArgumentException('Ambiguous key "'.implode('", and "', $diff).'".');
			}
