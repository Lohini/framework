...
extends \Nette\Object
{
	...
	public function __construct($url=NULL, $method=Request::GET)
	{
		if (!function_exists('curl_init')) {
			throw new \Nette\NotSupportedException('cURL is not supported by server.');
			}
		$this->setUrl($url);
	...
	public function setPost($post=array(), array $files=NULL)
	{
		if ($files) {
			if (!is_array($post)) {
				throw new \Nette\NotSupportedException('Not implemented.');
				}
	...
	{
		$this->error= $this->errorNumber= $this->info= $this->response= $this->responseHeaders= $this->requestHeaders= NULL;

		// method shouldn't be GET, when posting data
		if ($this->method===Request::GET && isset($this->options['postFields'])) {
			throw new \Nette\InvalidStateException('Method GET cannot send POST data or files.');
			}
