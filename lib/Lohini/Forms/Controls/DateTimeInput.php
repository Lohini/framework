...
extends \Nette\Forms\Controls\BaseControl
{
	...
	public function getFormat()
	{
		...
		return implode(
				'',
				array_map(
					function($part) use ($formats) {
						if (!isset($formats[$part])) {
							throw new \Nette\InvalidStateException("Format for $part is missing, please provide it writing in \$control->format['$part'].");
							}
						return $formats[$part];
						},
					$this->getParts()
	...
	protected function combineValue(array $value)
	{
		return implode(
				'',
				array_map(
					function($part) use ($value) {
						if (!isset($value[$part])) {
							throw new \Nette\InvalidStateException("Value of $part is missing.");
							}
						return $value[$part];
					},
				$this->getParts()
	...
	public function getControl()
	{
		...
		foreach ($this->getParts() as $part) {
			...
			$control->data('lohini-format', strtr($this->format[$part], static::$jsFormat));
	...
	/********************* Validation rules ************************/
	/**
	 * @param DateTimeInput|int|string $time1
	 * @param DateTimeInput|int|string $time2
	...
	/**
	 * @param DateTimeInput|int|string $time
	 * @return \DateTime|NULL
	 */
	...
	public static function register()
	{
		Container::extensionMethod(
			'addDate',
			function(Container $container, $name, $label=NULL) {
