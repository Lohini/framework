...
//		$name=get_class($this); // ...FrameworkPackage\FrameworkPackage
		$name=$this->getNamespace(); // ...Package\Framework
		$pos=strrpos($name, '\\');
	...
	public function getMigrations()
	{
		...
		foreach ($files=Finder::findFiles('Version*.php', 'Version*.sql')->in($dir) as $file) {
			...
			if ($refl->isSubclassOf('Lohini\Database\Migrations\AbstractMigration') && !$refl->isAbstract()) {
				$migrations[]=$class;
				}
			}
	 ...
	 */
	public function registerPresenters(\Nette\DI\ContainerBuilder $container)
	{
		if (!$dir=realpath($this->getPath().'/Presenters')) {
			return;
			}

		$ns=$this->getNamespace().'\\Presenters';
		foreach (Finder::findFiles('*Presenter.php')->from($dir) as $file) {
			...
			$id=\Lohini\Application\PresenterManager::formatPackagePresenterFromClass($refl->getName(), $this);
